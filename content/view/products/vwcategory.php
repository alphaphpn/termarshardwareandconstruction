<?php
	...
	$categoryv = trim($_GET['category']);
	$qrycategflt = "SELECT * FROM tblitem WHERE category=:categoryg AND deletedx=0 ORDER BY category ASC";
	$stmtcategflt = $cnn->prepare($qrycategflt);
	$stmtcategflt->bindValue(":categoryg", $categoryv);
	$stmtcategflt->execute();
	$num = $stmtcategflt->rowCount();
	$xno = 0;
