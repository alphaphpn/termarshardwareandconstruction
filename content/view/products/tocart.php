<?php
	...
	try {
		$temidid = isset($_GET['itemid']) ? $_GET['itemid'] : '<script>window.open("../../../routes/products/","_self");</script>';
		$nowuid = isset($_GET['uid']) ? $_GET['uid'] : '<script>window.open("../../../routes/products/","_self");</script>';
		$nqty = isset($_GET['gqty']) ? $_GET['gqty'] : '<script>window.open("../../../routes/products/","_self");</script>';
		...
		if ($cnt2 > 0) {
				...
				if ($cnt9 > 0) {
					...
					if ($fqty9plus>$cstock9) {
						$qry10 = "UPDATE tbl_order_item SET 
							...
							";
						$cnn->exec($qry10);
						echo '<script>alert("Already exceed on available stock! '.$cstock9.'available."); window.open("../../../routes/products/","_self");</script>';
					} else {
						$qry10 = "UPDATE tbl_order_item SET 
							qty			= '$fqty9plus', 
							total_amt	= '$totalamt9', 
							...
							";
						$cnn->exec($qry10);
						echo '<script>alert("You have added quantity of '.$fqty8.' and a total quantity '.$fqty9plus.'"); window.open("../../../routes/products/","_self");</script>';
					}
				} else {
					$qry11 = "INSERT INTO tbl_order_item SET 
						...
						";
					$cnn->exec($qry11);
					echo '<script>alert("You have added new item to your cart"); window.open("../../../routes/products/","_self");</script>';
				}
			} else {
				echo '<script>alert("No available item."); window.open("../../../routes/products/","_self");</script>';
			}
		} else {
			...
			if ($cntGetCUser > 0) {
				...
				echo '<script>alert("Added new transaction."); window.open("../../../routes/products/","_self");</script>';
			}
